async function inital() {
  ...
  if (updateTime["search-last-update"]) {
    //处理时间 精确到年月日时分秒
    let date = new Date(
      Number(updateTime["search-last-update"])
    ).toLocaleString();
    document.getElementById("search-last-update").innerText =
      "查老师数据更新时间：" + date;
  } else {
    document.getElementById("search-last-update").innerText =
      "查老师数据更新时间：未知";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("popup-setting")
    .addEventListener("click", handleSetting);
});
