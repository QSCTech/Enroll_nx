export default async function bookDisplay() {
    ...
    while (targetRows.length === 0 && currentTime < maxWaitTime) {
        const rows = Array.from(document.getElementsByClassName("ui-widget-content jqgrow ui-row-ltr"));
        rows.forEach(row => {
            targetRows.push(row);
        });
    ...
    await Promise.all(targetRows.map(async (row, index) => {
        const data = 'xkkh=' + row.id;
        const url = 'http://zdbk.zju.edu.cn/jwglxt/xsxjc/xsxjc_cxJcxx.html?time=' + Date.now() + '&gnmkdm=N253535&su=' + su;
        ...
        try {
            ...
            if (text[1] === '无教材' || text[1] === '作者') {
                ...
                text.forEach(word => {
                    if (word === '教材名称' || word === '作者' || word === '出版社' || word === '版本') {
                        bookInfo += word + '：';
                    } else {
                        bookInfo += word + '\n';
                    }
